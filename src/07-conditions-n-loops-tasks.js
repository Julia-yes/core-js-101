...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const res = arr.filter((item) => item === arr[i]);
    if (res.length === 1) {
      return arr[i];
    }
  }
...
function getDigitalRoot(num) {
  const res = [...num.toString()];
  const sumOfNum = res.reduce((sum, current) => sum + parseFloat(current), 0);
  if (sumOfNum > 9) {
    return Math.floor(sumOfNum / 10) + (sumOfNum - Math.floor(sumOfNum / 10) * 10);
  }
  return sumOfNum;
}
